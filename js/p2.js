...
jQuery(function($) {
	...
	if (isAsidePage || isP2CustomPostType) {
		...
		$(".entry-content > ul#postlist > li").each(function() {
		...
		//REVIEW does it work for single page now
		jQuery('li.' + customPostType).each(function() { bindActions(this, 'post'); });
		...
		if (isAsidePage && prologueTagsuggest && isUserLoggedIn)
	...
	function getPosts(showNotification){
		...
		ajaxCheckPosts = $.getJSON(queryString, function(newPosts){
			if (newPosts != null) {
				...
				if (!isAsidePage || (typeof newPosts.html == "undefined") ) {
					...
					$("#content > ul > li:first").before(newPosts.html);
					var newUpdatesLi = $("#content > ul > li:first");
					...
					if (counter >= newPosts.numberofnewposts && showNotification) {
						var updatemsg = isElementVisible('#content > ul >li:first') ? "" :  "<a href=\"#\"  onclick=\"jumpToTop();\" \">"+p2txt.jump_to_top+"</a>" ;
	...
	*/
	function newPost(trigger) {
		...
		var thisFormElements = $('#posttext, #default_category, #tags, :input',thisForm).not('input[type=hidden]');
		...
		var custom_post_type_cat = $('#default_category').val();

		var args = {action: 'new_post', _ajax_post:nonce, posttext: posttext, tags: tags, post_cat: post_cat, post_title: post_title, post_citation: post_citation, custom_post_type_cat: custom_post_type_cat };
		var errorMessage = '';
		$.ajax({
			type: "POST",
			url: ajaxUrl,
			...
			success: function(result) {
				...
				if (isAsidePage && result != "0") {
					getPosts(false);
				} else if (!isAsidePage && result != "0") {
	...
	*/
			...
			case "comment" :
				...
				if (inlineEditComments != 0 && isUserLoggedIn) {
					...
					thisCommentEditArea.editable(ajaxUrl, {event: 'edit', loadurl: ajaxUrl + '&action=get_comment&_inline_edit=' + nonce,
						...
						//REVIEW Image Path
						indicator : '<img src="' + styleSheetDir +'/p2/i/indicator.gif">', loadtext: p2txt.loading, cancel: p2txt.cancel,
			...
			case "post" :
				...
				if (inlineEditPosts != 0 && isUserLoggedIn) {
					...
					thisPostEditArea.editable(ajaxUrl, {event: 'edit', loadurl: ajaxUrl + '&action=get_post&_inline_edit=' + nonce,
						...
						//REVIEW Image Path
						indicator : '<img src="' + styleSheetDir +'/p2/i/indicator.gif">', loadtext: p2txt.loading,
	...
	function removeYellow() {
		...
		if (isAsidePage) {
			$('#content > ul > li.newupdates').each(function() {
				if (isElementVisible(this)) {
