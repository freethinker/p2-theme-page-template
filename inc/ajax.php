...
class P2Ajax {
	...
	function new_post() {
		...
		$custom_post_type_cat =  $_POST['custom_post_type_cat'];
		$custom_post_type_cat = &get_category($custom_post_type_cat);
		...
		$accepted_post_cats = apply_filters( 'p2_accepted_post_cats', array( $p2_custom_post_type, 'quote', 'status', 'link' ) );
		...
		if (!empty($custom_post_type_cat)) {
			$post_cat = $custom_post_type_cat;
		}
