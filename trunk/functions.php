<?php
define( 'P2_INC_PATH',  get_stylesheet_directory() . '/p2/inc' );
define( 'P2_INC_URL', get_stylesheet_directory_uri() .'/p2/inc' );
define( 'P2_JS_PATH',  get_stylesheet_directory() . '/p2/js' );
define( 'P2_JS_URL', get_stylesheet_directory_uri() .'/p2/js' );
...
function latest_post_permalink() {
	...
	$sql = "SELECT ID FROM {$wpdb->posts} WHERE post_type = {$p2_custom_post_type} AND post_status = 'publish' ORDER BY post_date DESC LIMIT 1";
...
function p2_title($before = '<h2>', $after = '</h2>', $returner = false) {
	...
	if( is_page() && !$is_page_template_page ) return;
...
function prologue_new_post_noajax() {
	global $p2_custom_post_type;
	if( 'POST' != $_SERVER['REQUEST_METHOD'] || empty( $_POST['action'] ) || $_POST['action'] != $p2_custom_post_type )
	    return;
